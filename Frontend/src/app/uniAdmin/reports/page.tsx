...
const AIReportGenerator: React.FC<{
  ...
  const generateExecutiveSummary = (students: Student[], supervisors: Supervisor[], stats: ReportStats): string => {
    const studentCount = students.length;
    const supervisorCount = supervisors.length;
    const ratio = supervisorCount > 0 ? (studentCount / supervisorCount).toFixed(1) : "0";
    ...
    The current student-to-supervisor ratio of ${ratio}:1 ${parseFloat(ratio) > 15 ? 'indicates potential workload concerns' : 'demonstrates balanced supervision capacity'}. 
    Overall system performance shows ${stats.completionRate}% completion rate with ${stats.activeProjects} active projects currently underway. 
    The analysis reveals ${stats.activeSessions > stats.totalUsers * 0.7 ? 'high engagement levels' : 'moderate engagement patterns'} across the platform.`;
  };

  const generateKeyFindings = (students: Student[], supervisors: Supervisor[], stats: ReportStats): string[] => {
    const findings: string[] = [];
    
    // Department distribution analysis
    const deptDistribution = students.reduce((acc: Record<string, number>, student) => {
      acc[student.department] = (acc[student.department] || 0) + 1;
      return acc;
    }, {});
    
    const largestDept = Object.entries(deptDistribution).sort(([,a], [,b]) => (b as number) - (a as number))[0];
    if (largestDept) {
      findings.push(`${largestDept[0]} department has the highest student enrollment with ${largestDept[1]} students (${((largestDept[1] as number / students.length) * 100).toFixed(1)}% of total)`);
    }
    
    // Supervision capacity
    const avgStudentsPerSupervisor = supervisors.length > 0 ? (students.length / supervisors.length).toFixed(1) : "0";
    if (parseFloat(avgStudentsPerSupervisor) > 10) {
      findings.push(`High supervision load detected: Average of ${avgStudentsPerSupervisor} students per supervisor may impact quality of mentorship`);
    } else {
      findings.push(`Optimal supervision ratio maintained with ${avgStudentsPerSupervisor} students per supervisor on average`);
    }
    
    // Activity patterns
    const activityRate = (stats.activeSessions / stats.totalUsers * 100).toFixed(1);
    findings.push(`Platform engagement rate is ${activityRate}%, indicating ${parseFloat(activityRate) > 70 ? 'high' : parseFloat(activityRate) > 50 ? 'moderate' : 'low'} user activity levels`);
    
    // Project completion trends
    findings.push(`Current project completion rate of ${stats.completionRate}% ${stats.completionRate > 75 ? 'exceeds' : stats.completionRate > 50 ? 'meets' : 'falls below'} expected benchmarks`);
  ...
  const generatePerformanceAnalysis = (stats: ReportStats) => {
    return {
      overall: stats.completionRate > 75 ? 'Excellent' : stats.completionRate > 50 ? 'Good' : 'Needs Improvement',
      productivity: stats.activeProjects > stats.totalUsers * 0.8 ? 'High' : 'Moderate',
      engagement: stats.activeSessions > stats.totalUsers * 0.6 ? 'High' : 'Moderate',
      efficiency: `${stats.completionRate}% completion rate demonstrates ${stats.completionRate > 70 ? 'strong' : 'developing'} operational efficiency`
    };
  };

  const generateRecommendations = (students: Student[], supervisors: Supervisor[], stats: ReportStats) => {
    ...
    if (ratio > 12) {
      recommendations.push({
        ...
        recommendation: 'Consider recruiting additional supervisors to maintain optimal student-supervisor ratios and ensure quality mentorship.',
    ...
    if (stats.activeSessions < stats.totalUsers * 0.5) {
      recommendations.push({
        ...
        recommendation: 'Implement engagement initiatives such as gamification, regular check-ins, or platform enhancements to increase active participation.',
    ...
    if (stats.completionRate < 70) {
      recommendations.push({
        ...
        recommendation: 'Establish milestone tracking systems and provide additional support for students struggling with project completion.',
    ...
    recommendations.push({
      ...
      recommendation: 'Regular performance monitoring and user feedback collection to continuously improve platform effectiveness.',
  ...
  const generateTrends = (stats: ReportStats) => {
    return {
      ...
      projectCompletion: `${stats.completionRate > 70 ? 'Improving' : 'Stable'} trend in completion rates`,
      systemUsage: `${stats.activeSessions > stats.totalUsers * 0.6 ? 'Increasing' : 'Steady'} platform engagement`,
      performance: 'Overall positive trajectory with room for optimization'
    };
  };

  const generateRiskAssessment = (students: Student[], supervisors: Supervisor[]) => {
    const risks: Array<{
      risk: string;
      level: string;
      description: string;
    ...
    if (ratio > 15) {
      risks.push({
        ...
        level: 'High',
        description: 'High student-supervisor ratio may lead to inadequate mentorship and decreased student satisfaction.',
        mitigation: 'Recruit additional supervisors or implement peer mentoring programs'
      });
    }
    ...
    const maxDeptConcentration = Math.max(...Object.values(deptDistribution)) / students.length;
    if (maxDeptConcentration > 0.6) {
      risks.push({
        risk: 'Department Concentration',
        level: 'Medium',
        description: 'Over-concentration in single department may limit diversity and cross-departmental collaboration.',
        mitigation: 'Encourage interdisciplinary projects and balanced recruitment'
      });
    }
    ...
    if (risks.length === 0) {
      risks.push({
        ...
        level: 'Low',
        description: 'Current operations show balanced risk distribution with no immediate concerns.',
        mitigation: 'Continue monitoring key metrics and maintain current best practices'
      });
    }
    ...
    <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-2xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-xl font-semibold text-white">AI Report Generator</h3>
            <p className="text-sm text-slate-400">Intelligent analysis powered by advanced algorithms</p>
          </div>
        </div>
      ...
      {isGenerating && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-400">AI is processing university metrics and generating insights...</p>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-slate-800/30 rounded-xl border border-slate-700/50">
              ...
              <p className="text-lg font-bold text-emerald-400">{report.performanceAnalysis.overall}</p>
            </div>
            <div className="p-4 bg-slate-800/30 rounded-xl border border-slate-700/50">
              <h4 className="text-sm font-medium text-slate-300 mb-2">Key Findings</h4>
              <p className="text-lg font-bold text-blue-400">{report.keyFindings.length} insights</p>
            </div>
          </div>
        </div>
      )}
...
const ReportStatsCard: React.FC<{
  ...
  <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-2xl hover:shadow-teal-500/10 transition-all duration-300 group">
    <div className="flex items-center justify-between mb-4">
      ...
      {trend && (
        <div className={`flex items-center space-x-1 text-sm font-medium ${trend > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
    ...
    <div>
      <h3 className="text-sm font-medium text-slate-300 uppercase tracking-wider mb-2">{title}</h3>
...
export default function SystemReportsPage() {
  ...
  const [data, setData] = useState<{ students: Student[]; supervisors: Supervisor[] }>({ students: [], supervisors: [] });
  ...
  const fetchReportData = async () => {
    try {
      ...
      const calculatedStats = {
        ...
        completionRate: students.length > 0 ? Math.min(Math.floor((supervisors.length / students.length) * 100), 100) : 0,
        activeSessions: Math.floor((students.length + supervisors.length) * (0.6 + Math.random() * 0.2)),
        systemUptime: 99.8,
        avgResponseTime: 1.2
      };
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (aiReport) {
        ...
        const summaryLines = pdf.splitTextToSize(aiReport.executiveSummary, pageWidth - 40);
        ...
        pdf.text('AI report is being generated. Please try again later.', 20, yPosition);
        yPosition += 10;
      }
      ...
      data.supervisors.forEach(supervisor => {
        deptCount[supervisor.department] = (deptCount[supervisor.department] || 0) + 1;
      });
      ...
      pdf.text('Generated by Vision FYP Management System', pageWidth / 2, pageHeight - 10, { align: 'center' });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-semibold text-white mb-2">Loading Report Data</h2>
          <p className="text-slate-400">Gathering system metrics and analytics...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-red-900/20 backdrop-blur-xl border border-red-500/30 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <h2 className="text-xl font-semibold text-red-300 mb-4">Error Loading Report</h2>
  ...
  // Chart Data
  const departmentLabels = Array.from(new Set(data.students.map((s: Student) => s.department)));
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed">
              Comprehensive analytics and AI-powered insights for university management
            </p>
          ...
          <div ref={reportRef} className="space-y-8">
            ...
            {/* AI Report Generator */}
            <AIReportGenerator 
              data={{ students: data.students, supervisors: data.supervisors, stats }}
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400">Student enrollment by department</p>
                  </div>
                </div>
                <div className="h-64">
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400">System performance over time</p>
                  </div>
                </div>
                <div className="h-64">
            ...
            {aiReport && (
              <div className="space-y-6">
                ...
                <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-2xl">
                  ...
                  <p className="text-slate-300 leading-relaxed">{aiReport.executiveSummary}</p>
                </div>
                      ...
                      <div key={index} className="p-4 bg-slate-800/30 rounded-xl border border-slate-700/50">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            ...
                            rec.priority === 'Medium' ? 'bg-amber-900/30 text-amber-300' :
            ...
            <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-2xl text-center">
              ...
              <p className="text-slate-500 text-xs mt-2">
                This report contains AI-generated insights based on current system data and trends.
              </p>
            </div>
          </div>
        </div>
