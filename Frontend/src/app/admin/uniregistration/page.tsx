  ...
  const validateField = (name: string, value: string): string => {
    switch (name) {
      case "shortName":
        ...
        if (value.length < 2 || value.length > 10) return "Short name must be 2-10 characters";
        if (!/^[A-Za-z0-9]+$/.test(value)) return "Short name can only contain letters and numbers";
        return "";
      ...
      case "phone":
      case "adminPhone":
        if (value && !/^[\+]?[0-9\s\-\(\)]{10,15}$/.test(value)) return "Invalid phone format";
        return "";
      ...
      case "maxSupervisors":
        ...
        if (name === "maxStudents" && num > 10000) return "Maximum 10,000 students allowed";
        if (name === "maxSupervisors" && num > 1000) return "Maximum 1,000 supervisors allowed";
        return "";
